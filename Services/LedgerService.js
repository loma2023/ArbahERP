...
// ═══════════════════════════════════════════════════════════
//    Source Document Models (for fetching actual numbers)
// ═══════════════════════════════════════════════════════════
const PaymentReceipt = mongoose.models.PaymentReceipt;
const PaymentVoucher = mongoose.models.PaymentVoucher;
const InvoiceSale = mongoose.models.InvoiceSale;
const InvoicePurchase = mongoose.models.InvoicePurchase;
const StockIn = mongoose.models.StockIn;
const StockOut = mongoose.models.StockOut;
const ReturnSale = mongoose.models.ReturnSale;
const ReturnPurchase = mongoose.models.ReturnPurchase;
...
// ═══════════════════════════════════════════════════════════
//    Helper: Get Actual Document Number from Source Collection
// ═══════════════════════════════════════════════════════════
const getSourceDocumentNumber = async (source, sourceId) => {
  ...
  try {
    ...
    switch (source) {
      case 'payment_receipt':
        if (PaymentReceipt) {
          doc = await PaymentReceipt.findById(sourceId).select('receiptNo receiptNumber number').lean();
          return doc?.receiptNo || doc?.receiptNumber || doc?.number || '';
        }
        break;
      case 'payment_voucher':
        if (PaymentVoucher) {
          doc = await PaymentVoucher.findById(sourceId).select('voucherNo voucherNumber number').lean();
          return doc?.voucherNo || doc?.voucherNumber || doc?.number || '';
        }
        break;
      case 'invoice_sale':
        if (InvoiceSale) {
          doc = await InvoiceSale.findById(sourceId).select('invoiceNo invoiceNumber number').lean();
          return doc?.invoiceNo || doc?.invoiceNumber || doc?.number || '';
        }
        break;
      case 'invoice_purchase':
        if (InvoicePurchase) {
          doc = await InvoicePurchase.findById(sourceId).select('invoiceNo invoiceNumber number').lean();
          return doc?.invoiceNo || doc?.invoiceNumber || doc?.number || '';
        }
        break;
      case 'stock_in':
        if (StockIn) {
          doc = await StockIn.findById(sourceId).select('number docNo').lean();
          return doc?.number || doc?.docNo || '';
        }
        break;
      case 'stock_out':
        if (StockOut) {
          doc = await StockOut.findById(sourceId).select('number docNo').lean();
          return doc?.number || doc?.docNo || '';
        }
        break;
      case 'return_sale':
        if (ReturnSale) {
          doc = await ReturnSale.findById(sourceId).select('number returnNo').lean();
          return doc?.number || doc?.returnNo || '';
        }
        break;
      case 'return_purchase':
        if (ReturnPurchase) {
          doc = await ReturnPurchase.findById(sourceId).select('number returnNo').lean();
          return doc?.number || doc?.returnNo || '';
        }
        break;
      default:
    ...
    // If model doesn't exist or any error, return empty
    return '';
  }
  return '';
};
...
const enrichLines = async (lines) => {
  const enriched = [];
  for (const line of lines) {
    // Fetch actual document number from source collection
    const sourceDocNo = await getSourceDocumentNumber(
      line.journalEntry?.source,
    ...
    enriched.push({
      ...
      documentNumber: sourceDocNo || line.journalEntry?.sourceId || '',
...
const getAccountLedger = async (accountId, fromDate, toDate, branch) => {
  ...
  const enrichedLines = await enrichLines(nonOpeningLines);
